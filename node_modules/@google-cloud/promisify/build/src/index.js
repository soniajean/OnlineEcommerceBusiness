    ...
    const methods = ownPropertyNames.filter(methodName => {
        ...
        return (
            ...
            !exclude.includes(methodName) &&
                typeof Class.prototype[methodName] === 'function' && !/(^_|(Stream|_)|promise$)|^constructor$/.test(methodName))
        );
        // clang-format on
    });
...
Class, options) {
    ...
    const methods = ownPropertyNames.filter(methodName => {
        ...
        return (
            ...
            (// is it a function?
            !exclude.includes(methodName) &&
                typeof Class.prototype[methodName] === 'function' && !/^_|(Stream|_)|^constructor$/.test(methodName))
        );
        // clang-format on
    });
    methods.forEach(methodName => {
